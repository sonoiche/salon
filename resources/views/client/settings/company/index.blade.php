        ...
        <div class="card custom-card">
            <div class="card-header d-sm-flex d-block">
                <ul class="nav nav-tabs nav-tabs-header mb-0 d-sm-flex d-block" role="tablist">
                    ...
                    <li class="nav-item m-1">
                        <a class="nav-link active" href="{{ url('client/settings/company') }}" aria-selected="true">Salon</a>
                    </li>
                </ul>
            </div>
            <div class="card-body">
                <div class="tab-content">
                    ...
                    @include('client.settings.tabs.company', ['active' => true])
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
{!! JsValidator::formRequest('App\Http\Requests\Client\CompanyRequest') !!}
<script>
...
function removeLogo() {
    if(confirm('Are you sure you want to delete this salon logo?')) {
        $.ajax({
            type: "DELETE",
            url: "{{ url('client/settings/company') }}/" + 0,
            dataType: "json",
            success: function (response) {
                location.reload();
            }
